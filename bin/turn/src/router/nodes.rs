...
pub struct Nodes {
    ...
    bonds: RwLock<HashMap<String, Addr>>,
}
...
impl Nodes {
    ...
    /// ```
    pub async fn get_node(&self, u: &str) -> Option<Node> {
        let addr = self.bonds.read().await.get(u)?.clone();
    ...
    /// ```
    pub async fn get_bond(&self, u: &str) -> Option<Addr> {
        self.bonds
            .read()
            .await
            .get(u)
