...
//! ## Event Handling
//!
//! The `TurnHooksServer` trait provides hooks for authentication and TURN server events:
//!
...
//!
//! gRPC transport types come from the `tonic` crate.
//!
//! ## See Also
//!
...
use self::protos::{
    Identifier, PasswordAlgorithm, TurnAllocatedEvent, TurnChannelBindEvent,
    TurnCreatePermissionEvent, TurnDestroyEvent, TurnRefreshEvent, TurnRegisterRequest,
    TurnRegisterResponse, TurnServerInfo, TurnSession, TurnSessionStatistics,
...
impl<T: TurnHooksServer + 'static> TurnHooksService for TurnHooksServerInner<T> {
    async fn register(
        &self,
        request: Request<TurnRegisterRequest>,
    ) -> Result<Response<TurnRegisterResponse>, Status> {
        ...
        if let Ok(credential) = self
            ...
            .await
        {
            Ok(Response::new(TurnRegisterResponse {
