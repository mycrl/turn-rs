...
// ///   allocation, where the allocation is determined by the 5-tuple on which
// the ///   Send indication arrived;
// ///
...
//     let addr = match ctx.env.state.get_port_addr(peer.port()) {
//         None => return Ok(None),
//         Some(a) => a,
//     };

//     let port = match ctx.env.state.get_addr_port(&ctx.addr) {
//         None => return Ok(None),
//         Some(p) => p,
//     };

//     let interface = match ctx.env.state.get_interface(&addr) {
//         None => return Ok(None),
//         Some(p) => p,
//     };
...
//     pack.append::<XorPeerAddress>(SocketAddr::new(interface.external.ip(),
// port));     pack.append::<Data>(data);
//     pack.flush(None)?;
