...
// the ///   ChannelData message arrived;
// ///
// /// * the destination transport address is the transport address to which the
// ///   channel is bound;
// ///
// /// * the data following the UDP header is the contents of the data field of
...
//     let addr = ctx.env.state.get_channel_bind(&ctx.addr, data.number)?;
//     let interface = ctx.env.state.get_interface(&addr)?;
