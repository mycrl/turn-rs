...
impl Operationer {
    ...
    fn create_message(&mut self, method: Method) -> MessageEncoder<'_> {
    ...
    async fn read_message(&mut self) -> Result<Message<'_>> {
        let size = timeout(
    ...
    pub async fn allocate(&mut self) -> Result<u16> {
        {
            ...
            self.state.password = generate_password(
...

        {
            ...
            message.flush(Some(&self.state.password))?;
        ...
        message.verify(&self.state.password)?;
